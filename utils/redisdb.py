...
rediskeys = ['EngagementTime', 'ViewerCount']
...
def get_most_engaged():
    response = redisConnection.zrange('EngagementTime',0,9,True,True)
    return [dict(name=x[0], description=str(int(x[1]/1000))) for x in response]

def get_sorted_spiders():
    response = redisConnection.zrange('SORTEDSPIDERS',0,9,True,True)
    return [dict(name=x[0], description=str(int(x[1]/1000))) for x in response]
